...
class EXP THammerPull : public TTechnical, public Fontable {
    public:
		enum { undefined=-1 };
        EXP friend SMARTP<THammerPull> newHammerPull(bool hammer);
        virtual void accept(TScoreVisitor& visitor);
...
// the new functions
//______________________________________________________________________________
    STechnicals newTechnicals();
    STechnical newTechnical(TTechnical::type t);
    SHarmonic newHarmonic();
    SHammerPull newHammerPull(bool h=true);
    SBendWithBar newBendWithBar();
    SBend newBend();
    SFingering newFingering();
